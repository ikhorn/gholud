...
//------------------------------------------------------------------------------
//таблица для быстрого подсчета crc8
//------------------------------------------------------------------------------

const static uint8_pr_t CRC8_table[256] = {
...
//------------------------------------------------------------------------------
//таблица для быстрого расчета CRC16
//------------------------------------------------------------------------------

const static uint8_pr_t CRC16_table_Hi[256] = {
...
const static uint8_pr_t CRC16_table_Lo[256] = {
...
uint8_t crc_Count_Crc8(const uint8_t din[], ubase_t dsize)
//---выщитывает однобайтовую контрольную сумму
{
	uint8_t crc8_out;
	crc8_out = 0;
	for (uint16_t i = 0; i < dsize; i++)
		crc8_out = prm_Rd_B(&CRC8_table[crc8_out ^ *din++]);
	return crc8_out;
}


uint16_t crc_Count_Crc16(const uint8_t din[], ubase_t dsize)
//---выщитывает двобайтную контрольную сумму
{
	uint8_t CRC_16_Hi, CRC_16_Lo;
	uint8_t index_CRC_tab;
	...
	for (uint16_t i = 0; i < dsize; i++)
	{
		index_CRC_tab = CRC_16_Hi ^ *din++;
		CRC_16_Hi = CRC_16_Lo ^ (prm_Rd_B(&CRC16_table_Hi[index_CRC_tab]));
		CRC_16_Lo = prm_Rd_B(&CRC16_table_Lo[index_CRC_tab]);
	}
	return (CRC_16_Hi<<8 | CRC_16_Lo);
}


void crc_Count_NextCrc16(const uint8_t din[], ubase_t dsize, uint16_t* crc)
//---выщитывает двобайтную контрольную сумму с добавлением
{
	uint8_t CRC_16_Hi, CRC_16_Lo;
	uint8_t index_CRC_tab;
